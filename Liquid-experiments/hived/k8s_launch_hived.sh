...
cp /dfs/k8s/job-template-hived.yaml tmp-${JOB_NAME}.yaml
...
echo "Job ${JOB_NAME} submitted at $(date "+%H:%M:%S")"
...
while true; do
  if [[ $(kubectl get pods -n ${JOB_NAME} | grep Running) ]]; then
      echo ${JOB_NAME} 'launched, wait for completion' $(date "+%H:%M:%S")
      break
  fi
  sleep 3
done
...
while true; do
  if [[ $(kubectl get pods -n ${JOB_NAME} | grep Completed) ]]; then
      echo "${JOB_NAME} Some pods completed, stop the jobs $(date "+%H:%M:%S")"
      break
  fi
  sleep 3
done
...
#echo "Job ${JOB_NAME} completed at $(date "+%H:%M:%S")"
