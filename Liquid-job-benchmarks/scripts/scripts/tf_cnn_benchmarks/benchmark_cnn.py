...
def make_params_from_flags():
  ...
  if flag_values['variable_update'] == 'distributed_replicated':
      if not flag_values['job_name']:
          print('empty')
          _str = os.getenv('TF_CONFIG', '{}')
          import json 
          _config = json.loads(_str)
          if _config['cluster']:
              flag_values['ps_hosts'] = str(','.join(_config['cluster']['ps']))
              flag_values['worker_hosts'] = str(','.join(_config['cluster']['worker']))

          if _config['task']:
              flag_values['job_name'] = str(_config['task']['type'])
              flag_values['task_index'] = int(_config['task']['index'])
      print(flag_values)
